...
- [`requestLEScanBatch(...)`](#requestlescanbatch)
...
| ------------------- | --------------------------------------------------------------------------- | --------------------------------- |
| **`options`**       | <code><a href="#requestbledeviceoptions">RequestBleDeviceOptions</a></code> |                                   |
| **`batchCallback`** | <code>(results: ScanResult[]) =&gt; void</code>                             | Callback for batched scan results |
...
| **`enableBatching`**   | <code>boolean</code>                          | Enable batching of scan results to reduce bridge overhead when scanning many devices. When enabled, scan results are collected and delivered in batches at regular intervals. (Android only, default: true)                                               |
| **`batchInterval`**    | <code>number</code>                           | Interval in milliseconds for delivering batched scan results. Only applies when enableBatching is true. (Android only, default: 50)                                                                                                                       |
